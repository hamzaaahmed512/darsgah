...
export const libraryActionSchema = z.discriminatedUnion("action", [
  ...
  z.object({ action: z.literal("issue"), copy_id: id, ...borrower, due_date: z.string().date(), reservation_id: id.optional() }),
  ...
  z.object({ action: z.literal("return"), id, outcome: z.enum(["returned", "damaged", "lost"]), note: z.string().trim().max(500).optional() }),
  ...
  z.object({
    ...
    loan_days: z.coerce.number().int().min(1).max(90).optional(),
    max_loans: z.coerce.number().int().min(1).max(30).optional(),
    max_renewals: z.coerce.number().int().min(0).max(10).optional(),
    fine_per_day: money.max(10000),
    student_loan_days: z.coerce.number().int().min(1).max(90).optional(),
    student_max_loans: z.coerce.number().int().min(1).max(30).optional(),
    student_max_renewals: z.coerce.number().int().min(0).max(10).optional(),
    student_renewal_days: z.coerce.number().int().min(1).max(90).optional(),
    staff_loan_days: z.coerce.number().int().min(1).max(90).optional(),
    staff_max_loans: z.coerce.number().int().min(1).max(30).optional(),
    staff_max_renewals: z.coerce.number().int().min(0).max(10).optional(),
    staff_renewal_days: z.coerce.number().int().min(1).max(90).optional(),
  }),
  // ── New actions ───────────────────────────────────────────────────────────

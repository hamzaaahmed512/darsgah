...
export default async function BookInventoryPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  {!copies.length ? (
    <p className="text-sm text-muted">
      No copies have been added for this book.
    </p>
  ) : (
    <div className="overflow-x-auto rounded-2xl border border-outline/60">
      <table className="min-w-full text-left text-sm">
        <thead className="bg-slate-50/80 font-label text-xs uppercase tracking-[0.12em] text-muted">
          <tr>
            <th className="px-5 py-4">Copy ID</th>
            <th className="px-5 py-4">Status</th>
            <th className="px-5 py-4">Replacement cost</th>
        ...
        <tbody>
          {copies.map((copy) => {
            const loan = currentLoans.get(copy.id);
            return (
              <tr key={copy.id} className="border-t border-outline/50">
                ...
                <td className="px-5 py-4">
                  {copy.replacement_cost == null ? (
                    ...
                    `Rs ${Number(copy.replacement_cost).toLocaleString("en-PK")}`
                  )}
                </td>
                <td className="px-5 py-4">
                  {loan?.borrower_name ?? <span className="text-muted">—</span>}
                </td>
                <td className="px-5 py-4">
                  {loan?.due_date ? (

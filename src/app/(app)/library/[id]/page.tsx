  ...
  return <>
    ...
    <Card className="rounded-[30px] border border-outline/70 bg-white shadow-card">
      ...
      <CardContent className="pt-6">
        {!copies.length ? <p className="text-sm text-muted">No copies have been added for this book.</p> : <DataTable ariaLabel="Library copies" count={copies.length} itemLabel="copies"><DataTableHeader><tr><th className="px-6 py-4">Copy ID</th><th className="px-6 py-4">Status</th><th className="px-6 py-4">Replacement cost</th><th className="px-6 py-4">Borrower</th><th className="px-6 py-4">Due date</th></tr></DataTableHeader><tbody>{copies.map((copy) => { const loan = currentLoans.get(copy.id); return <DataTableRow key={copy.id}><td className="px-6 py-5 text-sm align-middle font-semibold text-slate-900">{copy.accession}</td><td className="px-6 py-5 text-sm align-middle">{canManage && copy.status !== "on_loan" ? <CopyStatusEditor copyId={copy.id} bookId={book.id} initialStatus={copy.status} /> : <CopyStatus status={copy.status} />}</td><td className="px-6 py-5 text-sm align-middle">{copy.replacement_cost == null ? <span className="text-muted">Not specified</span> : `Rs ${Number(copy.replacement_cost).toLocaleString("en-PK")}`}</td><td className="px-6 py-5 text-sm align-middle">{loan?.borrower_name ?? <span className="text-muted">—</span>}</td><td className="px-6 py-5 text-sm align-middle">{loan?.due_date ? new Date(loan.due_date).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" }) : <span className="text-muted">—</span>}</td></DataTableRow>; })}</tbody></DataTable>}
      </CardContent>
    </Card>
  </>;
}

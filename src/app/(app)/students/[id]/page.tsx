...
export default async function StudentProfilePage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ from?: string; to?: string }> }) {
  ...
  return <>
    <PageHeader eyebrow={student.admission_number} title={formatFullName(student.first_name, student.last_name)} description={formatGradeSection(student.grade_name, student.section_name) || "Unassigned"} actions={<>
    ...
    <section className={`mt-6 grid gap-6 ${limitedView ? "" : "xl:grid-cols-[minmax(0,1.4fr)_minmax(320px,0.8fr)]"}`}>
      <Card><CardHeader><CardTitle>Biodata</CardTitle><Badge tone={student.status.startsWith("pending") ? "yellow" : student.status === "archived" ? "gray" : "green"}>{student.status.replaceAll("_", " ")}</Badge></CardHeader><CardContent className="grid gap-4 sm:grid-cols-2">
        <Info label="Preferred name" value={student.preferred_name ?? "Not recorded"} /><Info label="Gender" value={student.gender ?? "Not recorded"} />{!limitedView ? <Info label="Religion" value={student.religion ?? "Not recorded"} /> : null}{!limitedView ? <Info label="Birth date" value={student.date_of_birth} /> : null}<Info label="Admission date" value={student.admission_date} /><Info label="Class assignment" value={formatGradeSection(student.grade_name, student.section_name) || student.class_name || "Unassigned"} />{!limitedView ? <><Info label="Father alive" value={student.father_alive === false ? "No" : "Yes"} /><Info label="Email" value={student.email ?? "Not recorded"} icon={<Mail className="h-4 w-4" />} /><Info label="Phone" value={student.phone ?? "Not recorded"} icon={<Phone className="h-4 w-4" />} /><div className="sm:col-span-2"><Info label="Address" value={student.address ?? "Not recorded"} /></div></> : null}
      </CardContent></Card>
      {!limitedView ? <Card><CardHeader><CardTitle>Guardians and emergency contact</CardTitle></CardHeader><CardContent className="space-y-3">{guardians.length ? guardians.map((guardian: any) => <div key={guardian.guardian_id} className="rounded-lg bg-surface-low p-4"><p className="font-semibold text-ink">{formatDisplayName(guardian.full_name)}{guardian.is_primary ? " (Primary)" : ""}</p><p className="text-sm text-muted">{guardian.relationship} • {guardian.phone}</p><p className="text-sm text-muted">{guardian.email ?? "No email"}</p><p className="mt-2 text-sm text-muted">Emergency: {formatDisplayName(guardian.emergency_contact_name) || "Not recorded"} {guardian.emergency_contact_phone ? `• ${guardian.emergency_contact_phone}` : ""}</p></div>) : <EmptyState title="No guardian recorded" description="Guardian and emergency contact details will appear here." className="min-h-40 p-5" />}</CardContent></Card> : null}
    </section>

    <Card className="mt-6"><CardHeader><CardTitle>Attendance history</CardTitle><form className="flex flex-wrap items-end gap-2 text-sm"><label className="grid gap-1 text-muted">From<input name="from" type="date" defaultValue={filters.from} className="rounded-lg border border-outline bg-surface px-2 py-1 text-ink" /></label><label className="grid gap-1 text-muted">To<input name="to" type="date" defaultValue={filters.to} className="rounded-lg border border-outline bg-surface px-2 py-1 text-ink" /></label><button className="rounded-lg bg-primary px-3 py-1.5 font-semibold text-white">Filter</button></form></CardHeader><CardContent><HistoryTable emptyTitle="No attendance records" emptyDescription="Attendance for this student will appear here after it is submitted." headers={["Date", "Class", "Status", "Note"]} rows={attendance.map((row: any) => [row.attendance_date, formatGradeSection(row.classes?.grades?.name, row.classes?.sections?.name) || row.classes?.name || "—", <Badge key="status">{row.status}</Badge>, row.note ?? "—"])} /></CardContent></Card>

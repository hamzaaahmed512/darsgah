  ...
  return <>
    ...
    <section className={`mt-6 grid gap-6 ${limitedView ? "" : "xl:grid-cols-[minmax(0,1.4fr)_minmax(320px,0.8fr)]"}`}>
      <Card><CardHeader><CardTitle>Biodata</CardTitle><Badge tone={student.status.startsWith("pending") ? "yellow" : student.status === "archived" ? "gray" : "green"}>{student.status.replaceAll("_", " ")}</Badge></CardHeader><CardContent className="grid gap-4 sm:grid-cols-2">
        <Info label="Preferred name" value={student.preferred_name ?? "Not recorded"} /><Info label="Gender" value={student.gender ?? "Not recorded"} />{!limitedView ? <Info label="Religion" value={student.religion ?? "Not recorded"} /> : null}{!limitedView ? <Info label="Birth date" value={student.date_of_birth} /> : null}<Info label="Admission date" value={student.admission_date} /><Info label="Class assignment" value={student.class_name ?? "Unassigned"} />{!limitedView ? <><Info label="Father alive" value={student.father_alive === false ? "No" : "Yes"} /><Info label="Email" value={student.email ?? "Not recorded"} icon={<Mail className="h-4 w-4" />} /><Info label="Phone" value={student.phone ?? "Not recorded"} icon={<Phone className="h-4 w-4" />} /><div className="sm:col-span-2"><Info label="Address" value={student.address ?? "Not recorded"} /></div></> : null}
      </CardContent></Card>

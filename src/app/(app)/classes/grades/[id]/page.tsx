  ...
  return <>
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(320px,0.7fr)]">
      <Card className="overflow-hidden rounded-[28px] border border-outline/70 bg-white shadow-card">
        ...
        <CardContent className="grid gap-5">{subjects.length ? <GradeSubjectManager gradeId={id} availableSubjects={academicData.subjects} linkedSubjectIds={subjectIds} linkedSubjects={subjects} /> : <><EmptyState title="No subjects in this grade" description="Select an existing subject or create a new one below." /><GradeSubjectManager gradeId={id} availableSubjects={academicData.subjects} linkedSubjectIds={subjectIds} /></>}</CardContent>
      </Card>

        ...
        <Card>
          <CardContent className="p-0">
            {!transactions.rows.length ? <EmptyState title="No transactions found" description="Try another period or record a new income or expense." className="m-5" /> : <div className="overflow-x-auto"><table className="min-w-full text-left text-sm">
              ...
              <tbody>{transactions.rows.map((row: any) => <tr key={row.id} className="border-t border-outline/60"><td className="whitespace-nowrap px-4 py-3 font-mono text-xs font-semibold text-primary">{row.receipt_number}</td><td className="px-4 py-3"><Badge tone={row.direction === "income" ? "green" : "red"}>{TRANSACTION_CATEGORY_LABELS[row.category as TransactionCategory] ?? row.category.replace(/_/g, " ")}</Badge><p className="mt-1 whitespace-nowrap text-xs capitalize text-muted">{row.source.replace(/_/g, " ")}</p></td><td className="px-4 py-3"><p className="whitespace-nowrap font-semibold text-ink">{row.student_name || row.party_name || "—"}</p>{row.admission_number ? <p className="whitespace-nowrap text-xs text-muted">{row.admission_number}</p> : null}<p className="max-w-xs truncate text-xs text-muted">{row.description}</p></td><td className={`whitespace-nowrap px-4 py-3 font-bold ${row.direction === "income" ? "text-success" : "text-danger"}`}>{row.direction === "income" ? "+" : "−"}{formatPKR(Number(row.amount))}</td><td className="whitespace-nowrap px-4 py-3 text-muted">{formatDatePK(row.transaction_date)}</td><td className="whitespace-nowrap px-4 py-3 text-muted">{row.recorded_by_name || "System"}</td><td className="whitespace-nowrap px-4 py-3 text-muted">{row.reference_number || "—"}</td></tr>)}</tbody>
            </table></div>}
          </CardContent>
        </Card>

...
export default function FeaturesPage() {
  return <div className="marketing-home"><ScrollReveal />
    <div data-reveal><PageHero eyebrow="Product features" title={<>Everything your team needs.<br /><span className="text-primary">Nothing they don&apos;t.</span></>} description="Darsgah gives each school role a focused workspace while keeping information connected behind the scenes." /></div>
    <section data-reveal className="marketing-container py-20">
      <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
        {groups.map(({ icon: Icon, title, text, points }, index) => <article data-reveal key={title} className={`marketing-card marketing-feature-card rounded-[22px] border border-slate-200 bg-white p-7 shadow-[0_8px_30px_rgba(15,23,42,0.04)] feature-tone-${index % 3}`}>
    ...
    <section data-reveal className="border-y border-slate-200 bg-slate-50"><div className="marketing-container grid gap-10 py-16 md:grid-cols-3">
    ...
    <div data-reveal><CTA title="See Darsgah with your school's workflow" description="A personal walkthrough will show how your team can move from fragmented processes to one shared system." /></div>
  </div>;
}

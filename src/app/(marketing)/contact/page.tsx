    ...
    <div className="marketing-container relative grid gap-14 lg:grid-cols-[0.85fr_1.15fr] lg:items-start">
      <div className="lg:sticky lg:top-28">
        ...
        <div className="mt-10 grid gap-5">
          <ContactLine icon={Mail} title="Email us" text="darsgah.help@gmail.com" />
          <ContactLine icon={MessageSquareText} title="Personal walkthrough" text="A focused demo shaped around your needs" />
          <ContactLine icon={Clock3} title="Response" text="We aim to reply within two business days" />
        </div>
      </div>
...
function ContactLine({ icon: Icon, title, text }: { icon: typeof Mail; title: string; text: string }) {
  return <div className="flex gap-4"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-blue-100 bg-primary-soft text-primary"><Icon className="h-4 w-4" /></span><div><p className="text-sm font-bold text-ink">{title}</p><p className="mt-1 text-sm text-muted">{text}</p></div></div>;
}

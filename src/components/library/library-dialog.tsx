...
export function LibraryDialog({ title, description, children, onClose, className = "max-w-lg" }: {
  ...
  return <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/40 p-0 backdrop-blur-sm sm:items-center sm:p-4" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}>
    <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby={`${id}-title`} aria-describedby={description ? `${id}-description` : undefined} className={`dialog-panel flex max-h-[calc(100dvh-0.75rem)] w-full flex-col overflow-hidden rounded-t-[28px] bg-white shadow-xl sm:max-h-[calc(100dvh-2rem)] sm:rounded-[28px] ${className}`}>

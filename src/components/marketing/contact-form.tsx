...
export function ContactForm() {
  ...
  return <form action={submit} className="grid gap-5">
    ...
    <label className="grid gap-2 text-sm font-bold text-ink"><span>How can we help?<span className="ml-0.5 text-danger" aria-hidden="true">*</span></span><textarea name="message" required aria-required="true" rows={5} placeholder="Tell us about your school and what you would like to improve." className="resize-none rounded-xl border border-outline bg-white px-4 py-3 text-sm font-normal text-ink outline-none placeholder:text-slate-400 focus:border-primary focus:ring-4 focus:ring-blue-100" /></label>
...
function Field({ label, name, type = "text", placeholder }: { label: string; name: string; type?: string; placeholder: string }) { return <label className="grid gap-2 text-sm font-bold text-ink"><span>{label}<span className="ml-0.5 text-danger" aria-hidden="true">*</span></span><input name={name} type={type} required aria-required="true" placeholder={placeholder} onChange={type === "email" ? (event) => { event.currentTarget.value = normalizeEmail(event.currentTarget.value); } : undefined} className="h-12 rounded-xl border border-outline bg-white px-4 text-sm font-normal text-ink outline-none placeholder:text-slate-400 focus:border-primary focus:ring-4 focus:ring-blue-100" /></label>; }

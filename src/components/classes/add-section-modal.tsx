    ...
    {open ? <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm">
      <div className="dialog-panel w-full max-w-md rounded-[28px] border border-outline/70 bg-white shadow-lift">
        ...
        <form onSubmit={submit} className="grid gap-6 bg-slate-50/30 p-6">
          ...
          <div className="flex flex-wrap justify-end gap-3"><Button type="button" variant="secondary" onClick={() => setOpen(false)}>Cancel</Button><Button disabled={pending}>{pending ? "Creating..." : "Create section"}</Button></div>
        </form>
      </div>
    </div> : null}
  </>;

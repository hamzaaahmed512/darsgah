...
import { CalendarDays, GraduationCap, NotebookPen, RotateCcw, Users } from "lucide-react";
      ...
      <section className="rounded-[26px] border border-outline/70 bg-white p-4 shadow-card sm:p-5">
        <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(280px,0.62fr)_auto] xl:items-end">
          ...
          <div className="flex w-full flex-wrap gap-2 xl:w-auto xl:flex-nowrap">
            {canUndo ? (
              <Button type="button" variant="secondary" onClick={undo} disabled={pending} className="min-h-12 flex-1 rounded-2xl px-4 text-sm xl:flex-none">
                <RotateCcw className="h-4 w-4" aria-hidden="true" />
                Undo & Edit
              </Button>
            ) : null}
            <Button onClick={submit} disabled={!canSubmit || pending || !roster.length} className="min-h-12 flex-1 rounded-2xl px-5 text-sm xl:flex-none">
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-1.5 text-sm text-muted">
                {editWindowOpen ? "Update the saved records and resubmit before the edit window closes." : submitted ? "Attendance already marked for today." : "Default status is present. Update exceptions before saving."}
              </p>
            </div>
          </div>

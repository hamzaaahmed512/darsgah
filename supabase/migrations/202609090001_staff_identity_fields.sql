...
as
select
  sm.id as member_id, sm.school_id, p.id as user_id, p.full_name, p.email, p.avatar_url,
  p.phone, p.bio, p.cnic, p.gender, sm.role, sm.status, sm.department, sm.job_title,
...
group by sm.id, sm.school_id, p.id, p.full_name, p.email, p.avatar_url, p.phone, p.bio, p.cnic, p.gender, sm.role, sm.status, sm.department, sm.job_title;
